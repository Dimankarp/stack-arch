...
#DataPath
from datapath import ALUOp, Datapath, RSPush, TOSLatch, PCLatch
from exceptions import EOFException, Halt, MicrocodeException
      ...
      match signal:
         ...
         case mPCLatch.IR:
            if 'opcode' in self._dp._IR:
               ...
               raise MicrocodeException("Failed to mjump by opcode")
   ...
   def simulate(self, tick_limit: int):
      try:
         while self._ticks < tick_limit:
            ...
            self._ticks+=1
            mInstructions = mProgram[self._mPC]

            #Will be rewritten on mjump
            self._mPC+=1 
            
            for instr in mInstructions:
      ...
      return output, self._ticks, 
   def __repr__(self) -> str:
      return f'TCK: {self._ticks:3} mPC: {self._mPC:3}\n mPROG: {mProgram[self._mPC]}'
